        ...
        data.timelineRows.asReversed().forEachIndexed { index, row ->
            ...
            when (row) {
                is ChatTimelineRow.Message -> item(
                    ...
                    val tight = originalIndex != dividerIndex && continuesMessageGroup(previous, message)
                    Column(modifier = Modifier.fillMaxWidth()) {
                ...
                is ChatTimelineRow.PresenceSummary -> item(
                    ...
                    val burstPrevious = data.timelineRows.getOrNull(originalIndex - 1)?.messages?.lastOrNull()
